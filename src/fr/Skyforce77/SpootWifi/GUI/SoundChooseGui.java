...
import fr.Skyforce77.SpootWifi.GUI.Widgets.SoundComboBox;
import fr.Skyforce77.SpootWifi.GUI.Widgets.ValueSlider;
import fr.Skyforce77.SpootWifi.Saves.SWStorage;

public class SoundChooseGui extends SWGui {
	
	public SoundChooseGui(String title, SpoutPlayer sp, SWStorage storage)
	{
		...
		texture.setHeight(190);
		texture.setPriority(RenderPriority.High);
		texture.setX(-texture.getWidth()/2);
		texture.setY(-texture.getHeight()/2);
		
		GenericComboBox color = new SoundComboBox(storage,sp);
		color.setText("Choose Sound");
		
		ValueSlider high = new ValueSlider(sp, storage, "SoundHigh");
		ValueSlider volume = new ValueSlider(sp, storage, "SoundVolume");
		
		container.addChildren(color,volume,high);
